      ...
      for (beta in beta_indexs){
        ...
        if (nums > 2){
          ...
          if (p_nums > 2){
            ...
            saveWidget(ggp1,file = paste0(resdir, '/', beta, '_', class, '_', specie,'_NMDS.html'), selfcontained = T)
            ggsave(paste0(resdir,'/', beta,'_', class, '_', specie,'_NMDS.pdf'),p1, width =7.5, height = 6, device = cairo_pdf)
            ggsave(paste0(resdir,'/', beta,'_', class, '_', specie,'_NMDS_labeled.pdf'),p2, width =7.5, height = 6, device = cairo_pdf)
            ggsave(paste0(resdir,'/', beta,'_', class, '_', specie,'_NMDS_ellipse.pdf'),p3, width =7.5, height = 6, device = cairo_pdf)
          }
        }
