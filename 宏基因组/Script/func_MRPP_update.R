    ...
    for (file in files){
      if (file.exists(file.path(table_dir, file))){
        ...
        if (type == '1.KEGG'|type == '2.eggNOG'|type =='3.CAZy'|type == '4.GO'){
          resdir = paste(res_dir, gro_num, '8-FunctionStatistical_analysis', type, '8.MRPP', sep='/')
        } else if (grepl('_Cycle', type)){
          resdir = paste(res_dir, gro_num, '9-METABOLIC', type, '6.Statistical_test_analysis', '8.MRPP', sep='/')
        } else if (type == 'ARG'){
          resdir = paste(res_dir, gro_num, '10-ARG','6.Statistical_test_analysis', '8.MRPP', sep='/')
        } else if(type == 'VFDB'){
          resdir = paste(res_dir, gro_num, '11-VFDB', '6.Statistical_test_analysis','8.MRPP', sep='/')
        } else if(type == 'mobileOG'){
          resdir = paste(res_dir, gro_num, '12-mobileOG', '6.Statistical_test_analysis','8.MRPP', sep='/')
        } else if(type == 'BacMet2'){
          resdir = paste(res_dir, gro_num, '13-BacMet2', '6.Statistical_test_analysis','8.MRPP', sep='/')
        } else if(type == 'QS'){
          resdir = paste(res_dir, gro_num, '14-QS', '6.Statistical_test_analysis','8.MRPP', sep='/')
        }
        
        if (!file.exists(resdir)){dir.create(resdir, recursive = T)}
        
        write.xlsx(MRPP_df, file = paste0(resdir,'/', prefix, '_MRPP.xlsx'))
