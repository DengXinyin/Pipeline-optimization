...
def run_lefse(task):
    tpm_dir, specie, log_dir = task
    tsv = os.path.join(tpm_dir, f'{specie}.tsv')
    infile = os.path.join(tpm_dir, f'{specie}.in')
    resfile = os.path.join(tpm_dir, f'{specie}.res')
    log_file = os.path.join(log_dir, f'{specie}.lefse.log')
    ...
    for i in range(1, k):
        ...
        for clas in CLASSES:
            ...
            for specie in SPECIES:
                infile = os.path.join(tax_dir, f'{specie}.xlsx')
                if not os.path.exists(infile):
                    continue

                out_tsv = os.path.join(tpm_dir, f'{specie}.tsv')
                ...
                tasks.append((tpm_dir, specie, lefse_log_dir))
    ...
    for i in range(1, k):
        ...
        for clas in CLASSES:
            ...
            if not os.path.exists(tpm_dir):
                continue
            for specie in SPECIES:
                infile = os.path.join(tpm_dir, f'{specie}.res')
                if not os.path.exists(infile):
                    continue
                ...
                try:
                    ...
                    res.to_csv(
                        os.path.join(tpm_dir, f'{specie}_LDA.tsv'),
...
def plot_lefse(tpmdir, datadir, res_dir):
    ...
    cmd = [Rscript_j, os.path.join(scripts_path, 'tax_LDAscore.R'), tpmdir, datadir, res_dir]
    log.info('运行 R 脚本: tax_LDAscore.R')
    with open(log_file, 'w') as lf:
