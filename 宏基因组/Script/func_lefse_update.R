...
for (i in 1: k){
  for (type in types){
    ...
    table_dir <- paste(tpm_dir, gro_num, 'lefse', type, sep = '/')
    
    files = list.files(table_dir, pattern = '_LDA.tsv')
    file = files[1]
    for (file in files){
      if (file.exists(file.path(table_dir, file))){
        prefix <- strsplit(file, '_LDA.tsv')
        LDA_tab <- read.csv(file.path(table_dir, file),
                            sep = '\t', check.names = F)
        ...
        if (type == '1.KEGG'|type == '2.eggNOG'|type =='3.CAZy'|type == '4.GO'){
          resdir = paste(res_dir, gro_num, '8-FunctionStatistical_analysis', type, '9.Lefse', sep='/')
        } else if (grepl('_Cycle', type)){
          resdir = paste(res_dir, gro_num, '9-METABOLIC', type, '6.Statistical_test_analysis', '9.Lefse', sep='/')
        } else if (type == 'ARG'){
          resdir = paste(res_dir, gro_num, '10-ARG','6.Statistical_test_analysis', '9.Lefse', sep='/')
        } else if(type == 'VFDB'){
          resdir = paste(res_dir, gro_num, '11-VFDB', '6.Statistical_test_analysis','9.Lefse', sep='/')
        } else if(type == 'mobileOG'){
          resdir = paste(res_dir, gro_num, '12-mobileOG', '6.Statistical_test_analysis','9.Lefse', sep='/')
        } else if(type == 'BacMet2'){
          resdir = paste(res_dir, gro_num, '13-BacMet2', '6.Statistical_test_analysis','9.Lefse', sep='/')
        } else if(type == 'QS'){
          resdir = paste(res_dir, gro_num, '14-QS', '6.Statistical_test_analysis','9.Lefse', sep='/')
        }
        if (!file.exists(resdir)){dir.create(resdir, recursive = T)}
        
        plot_nums = nrow(LDA_tab)
        if (plot_nums > 80){
          ggsave(paste0(resdir, '/', prefix,'_LDA.pdf'),p,
        ...
        saveWidget(ggp,file = paste0(resdir, '/', prefix,'_LDA.html'), selfcontained = T)
      }
    }
  }
}