        ...
        for label, table_dir, subitems in iterators:
            ...
            for sign_file, extra in files:
                ...
                if not os.path.exists(sign_path):
                    continue
                try:
                    sign_df = pd.read_csv(sign_path, sep='\t')
                except Exception as exc:
...
def _save_stamp_plots(mat, diff, resdir, out_prefix, width=11.0, height=7.0):
    ...
    for j in range(len(features) - 1):
        col = 'gray95' if j % 2 == 0 else 'white'
        for ax in (ax1, ax2):
    ...
    ci_low = plot_diff['conf.low'].fillna(0).values
    ci_high = plot_diff['conf.high'].fillna(0).values
    higher_group = plot_diff['Group'].values
    colors2 = [CBB_PALETTE[0] if g == g1 else CBB_PALETTE[1] for g in higher_group]

    ax2.errorbar(estimates, y, xerr=[estimates - ci_low, ci_high - estimates],
    ...
    figp.add_trace(go.Scatter(
        ...
        error_x=dict(type='data', symmetric=False, array=ci_high - estimates, arrayminus=estimates - ci_low),
        showlegend=False
    ), row=1, col=2)
    ...
    figp.add_annotation(x=0.5, y=0.5, text='P-value (corrected)', showarrow=False,
                        xref='x3 paper', yref='y3 paper', textangle=-90, font=dict(size=11))
    ...
    figp.write_html(os.path.join(resdir, f'{out_prefix}.html'), include_plotlyjs='cdn')
...
def _run_random_forest(mat, resdir, out_prefix, width=10.5, height=7.0):
    ...
    plot_tab = save_tab[save_tab['MeanDecreaseGini'] > 0].sort_values(
        'MeanDecreaseGini', ascending=False).head(10).reset_index().rename(columns={'index': 'tax'})
    if plot_tab.empty:
        return
        ...
        if kind == 'tax':
            for clas in CLASSES:
                ...
                for specie in SPECIES:
                    ...
                    r, p, df_plot = _anosim(squareform(dist_sq), dist_sq, groups)
                    if np.isnan(r):
                        continue
