...
# -*- coding: utf-8 -*-
# By: Wang Li 2024
# 20260618_original: 原 megahit.py 的精确副本，仅修复 Docker 挂载点 FileExistsError，
#                    用于和优化版 megahit_update.py 做运行时间对比。
...
def megahit(host, cleandir, host_dir, datadir, tmpdir, scripts_path):
    ...
    if host == 'none':
        cmd = '''
bash {0}/megahit_original.sh {1} {2} {3} 'none'
'''.format(scripts_path, datadir, cleandir, tmpdir)
    else:
        cmd = '''
        bash {0}/megahit_original.sh {1} {2} {3} 'host'
...
def main():
    ...
    # 修复：当 megahit_dir 是 Docker 挂载点时，不能删除目录本身，只清空内容
