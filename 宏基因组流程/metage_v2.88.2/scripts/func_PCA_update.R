  ...
  for (type in types){
      ...
      for (file in files){
        ...
        if (nums > 2){
          ...
          p2 <- p1 + geom_text_repel(label = pca_labels, size=3.7, max.overlaps = 50)
          ...
          if (type == '1.KEGG'| type == '2.eggNOG'|type =='3.CAZy'|type == '4.GO'){
            ...
            resdir = paste(res_dir, gro_num, '15-COG', '3.PCA', sep='/')
          } else if(type == 'MetaCyc'){
            resdir = paste(res_dir, gro_num, '16-MetaCyc', '3.PCA', sep='/')
          }
