...
for (i in 1: k){
  for (type in types){
    ...
    table_dir <- paste(tpm_dir, gro_num, 'wilcoxon', type, sep = '/')
    ...
    for (file in files){
      if (file.exists(file.path(table_dir, file))){
        ...
        for (j in 1:nums){
          ...
          row <- melt(row, id = c('pathway', 'statistic', 'p_value', 'padj'))
          ...
          # 保存结果
          if (type == '1.KEGG'){
            resdir = paste(res_dir, gro_num, '8-FunctionStatistical_analysis', type, '2.wilcoxon', prefix, sep='/')
          } else if (type == '2.eggNOG'|type =='3.CAZy'|type == '4.GO'){
            resdir = paste(res_dir, gro_num, '8-FunctionStatistical_analysis', type, '2.wilcoxon', sep='/')
          } else if (grepl('_Cycle', type)){
            resdir = paste(res_dir, gro_num, '9-METABOLIC', type, '6.Statistical_test_analysis', '2.wilcoxon', sep='/')
          } else if (type == 'ARG'){
            resdir = paste(res_dir, gro_num, '10-ARG','6.Statistical_test_analysis', '2.wilcoxon', sep='/')
          } else if(type == 'VFDB'){
            resdir = paste(res_dir, gro_num, '11-VFDB', '6.Statistical_test_analysis','2.wilcoxon', sep='/')
          } else if(type == 'mobileOG'){
            resdir = paste(res_dir, gro_num, '12-mobileOG', '6.Statistical_test_analysis','2.wilcoxon', sep='/')
          } else if(type == 'BacMet2'){
            resdir = paste(res_dir, gro_num, '13-BacMet2', '6.Statistical_test_analysis','2.wilcoxon', sep='/')
          } else if(type == 'QS'){
            resdir = paste(res_dir, gro_num, '14-QS', '6.Statistical_test_analysis','2.wilcoxon', sep='/')
          }
