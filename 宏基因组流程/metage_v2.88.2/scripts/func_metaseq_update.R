  ...
  for (type in types){
    ...
    for (file in files){
      if (file.exists(file.path(table_dir, file))){
        ...
        if (nrow(group) == 0 || ncol(otu$counts) == 0 || nrow(otu$counts) == 0){
          next
        }
        otu$counts <- otu$counts[, match(group[,1], colnames(otu$counts)), drop = FALSE]
        if (ncol(otu$counts) == 0 || nrow(otu$counts) == 0){
          next
        }
        result <- apply(otu$counts, 1, function(row) all(row == row[1]))
        otu$counts <- otu$counts[result == F, , drop = FALSE]
        if (nrow(otu$counts) == 0 || ncol(otu$counts) == 0){
          next
        }
