#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Update version of collect_res.py
...
def main():
    ...
    seen = set()
    for src in [args.res1, args.res2, args.res3, args.res4, args.res5]:
        src = os.path.abspath(src)
        if src in seen:
            log.info('跳过重复源目录: %s', src)
            continue
        seen.add(src)
...
if __name__ == '__main__':
    try:
        main()
        log.info('collect_res 完成')
    except Exception as e:
        log.error('collect_res 失败: %s', e)
        sys.exit(1)
