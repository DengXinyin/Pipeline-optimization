...
for (i in 1: k){
  for (class in classes){
    ...
    table_dir <- paste(tpm_dir, gro_num, 'anova', class, sep = '/')
    ...
    for (specie in species){
      ...
      if (file.exists(file.path(table_dir, file.name))){
        ...
        for (j in 1:nums){
          row <- genus_sign[j,]
          row <- melt(row, id = c(specie, 'F_value', 'p_value', 'padj'))
          ...
          p <- ggplot(data = row,aes(x=group, y=value, fill=group)) +
            ...
            ggtitle(paste0(genus.name,', pvalue=',p_value))
          ggp <- ggplotly(p)
          
          resdir <- paste(res_dir,gro_num, '6-TaxStatistical_analysis', class, specie,'1.ANOVA', sep = '/')
