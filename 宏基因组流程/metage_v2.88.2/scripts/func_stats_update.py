...
def get_table(anno_dir, datadir, res_dir, func_tmpdir, dbdir):
    ...
    for i in range(1, k):
        ...
        kegg_selected = [c for c in kegg_preferred if c in kegg_tpm_all.columns]
        # 数据库升级后新增的 KO/KEGG 注释列也必须进入交付表。
        kegg_selected += [
            c for c in kegg_tpm_all.columns
            if c not in samples_ls and c not in kegg_selected
        ]
