...
def get_result_dir(res_dir, group_num, func):
    if func in ('1.KEGG', '2.eggNOG', '3.CAZy', '4.GO'):
        ...
        raise ValueError('未知功能类型: {0}'.format(func))
...
def run_lefse(task):
    ...
    tsv = os.path.join(diff_dir, '{0}.tsv'.format(prefix))
    infile = os.path.join(diff_dir, '{0}.in'.format(prefix))
    resfile = os.path.join(diff_dir, '{0}.res'.format(prefix))
    log_file = os.path.join(log_dir, '{0}.lefse.log'.format(prefix))
    ...
    for i in range(1, k):
        ...
        for func in FUNC_INDEX:
            ...
            for file in os.listdir(table_dir):
                ...
                out_tsv = os.path.join(diff_dir, '{0}.tsv'.format(prefix))
        ...
        for func in FUNC_INDEX:
            ...
            for file in os.listdir(diff_dir):
                ...
                try:
                    ...
                    res.to_csv(
                        os.path.join(diff_dir, '{0}_LDA.tsv'.format(prefix)),
                        sep='\t', index=False, encoding='utf-8-sig'
                    )
                    res.to_excel(
                        os.path.join(resdir, '{0}_LDA.xlsx'.format(prefix)),
