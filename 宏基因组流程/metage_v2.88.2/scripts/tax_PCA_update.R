    ...
    for (class in classes){
      ...
      for (specie in species){
        ...
        if (nums > 2){
          ...
          pca_labels <- rownames(pca_mat)
          if (length(display_map) > 0) {
            pca_labels <- display_map[pca_labels]
            pca_labels[is.na(pca_labels)] <- rownames(pca_mat)[is.na(pca_labels)]
          }
          p2 <- p1 + geom_text_repel(label = pca_labels, size=4.4, max.overlaps = 50)
          ...
          resdir <- paste(res_dir, gro_num, '5-TaxAnnotation','6.Beta_diversity_analysis', class, specie, '1.PCA', sep = '/')
          if (!file.exists(resdir)){dir.create(resdir, recursive = T)}
          
          prefix_name <- paste0(class, '_', specie, '_PCA')
          saveWidget(ggp1,file = file.path(resdir, paste0(prefix_name, '.html')), selfcontained = T)
          ggsave(file.path(resdir,paste0(prefix_name, '.pdf')),p1, width =7.5, height = 6, device = cairo_pdf)
          ggsave(file.path(resdir,paste0(prefix_name, '_labeled.pdf')),p2, width =7.5, height = 6, device = cairo_pdf)
          ggsave(file.path(resdir,paste0(prefix_name, '_ellipse.pdf')),p3, width =7.5, height = 6, device = cairo_pdf)
          ...
          write.xlsx(sheets, file = file.path(resdir, 'PCA.xlsx'),rowNames =T)
        }
